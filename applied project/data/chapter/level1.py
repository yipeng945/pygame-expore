...
class Level1(tools._State):
    ...
    def setup_background(self):
        ...
        print("fefe", self.level_rect)

        self.viewport = setup.SCREEN.get_rect(bottom=self.level_rect.bottom)
        print("viewport", self.viewport)

        self.viewport.x = self.game_info[c.CAMERA_START_X]
        print("viewport x", self.viewport.x)

    def setup_mario(self):
        self.mario = mario.Mario()
        if self.persist[c.IS_FROM_COIN]:
            self.mario.rect.x = 6988
            self.mario.rect.y = 452
        else:
    ...
    def setup_pipes(self):
        ...
        pipe1 = collider.Collider(1200, 452, 83, 82)
    ...
    def setup_steps(self):
        ...
        step1 = collider.Collider(5725, 495, 40, 44)
        step2 = collider.Collider(5768, 452, 40, 44)
        step3 = collider.Collider(5811, 409, 40, 44)
        step4 = collider.Collider(5854, 366, 40, 176)

        step5 = collider.Collider(5981, 366, 40, 176)
        step6 = collider.Collider(6024, 408, 40, 40)
        step7 = collider.Collider(6067, 452, 40, 40)
        step8 = collider.Collider(6110, 495, 40, 40)

        step9 = collider.Collider(6325, 495, 40, 40)
        step10 = collider.Collider(6368, 452, 40, 40)
        step11 = collider.Collider(6411, 409, 40, 40)
        step12 = collider.Collider(6454, 366, 40, 40)
        step13 = collider.Collider(6497, 366, 40, 176)

        step14 = collider.Collider(6624, 366, 40, 176)
        step15 = collider.Collider(6667, 408, 40, 40)
        step16 = collider.Collider(6708, 452, 40, 40)
        step17 = collider.Collider(6751, 495, 40, 40)
    ...
    def setup_bricks(self):
        ...
        brick17 = bricks.Brick(4030, 365, c.FIVECOINS, self.coin_group)
        ...
        self.brick_group = pyg.sprite.Group(
            brick1,
            brick2,
            brick3,
            brick4,
    ...
    def update(self, surface, keys, current_time):
        ...
        # self.game_info[c.REMAIN_TIME] = self.
    ...
    def set_coin_room(self):
        print("mario", self.mario.rect.left)

        if self.mario.crouching == True:
            if (
                (self.mario.rect.x >= 1970 or self.mario.rect.x <= 2000)
                and self.mario.rect.y == 326
                or self.mario.rect.y == 311
            ):
                # self.persist[c.REMAIN_TIME] = self.persist[c.CURRENT_TIME]
                if self.mario.big:
                    self.persist[c.IS_MARIO_BIG] = True
                self.next = c.COIN_ROOM
                self.done = True
    ...
    def set_game_info_values(self):
        # print("mario", self.mario.rect.x, self.mario.rect.y)
    ...
    def end_game(self):
        if self.flag_timer == 0:
            ...
            # back to main menu
            self.next = c.GAME_OVER
            self.persist[c.IS_FROM_COIN] = False
            self.persist[c.IS_MARIO_BIG] = False
            self.persist[c.REMAIN_TIME] = 400
